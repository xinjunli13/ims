...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>
												<img onclick="ddpirnt('${var.OUTKU_ID}');" title="打印订单" alt="打印订单" src="static/images/print.png" width="20" style="margin-top: -5px;cursor:pointer;" />
												${var.ORDER_NUMBER}
											</td>
	...
	<script type="text/javascript">
		...
		function ddpirnt(OUTKU_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="打印订单";
			 diag.URL = '<%=basePath%>outku/ddpirnt.do?OUTKU_ID='+OUTKU_ID;
			 diag.Width = 450;
			 diag.Height = 399;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//导出excel
		function toExcel(){
			$("#Form").attr("action", "<%=basePath%>outku/excel.do");
			setTimeout(function() {
				$("#Form").attr("action", "<%=basePath%>outku/list.do");
	        }, 500 );
			$("#Form").submit();
		}
	</script>
